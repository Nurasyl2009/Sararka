...
import { Plus, Pencil, Trash2, FileText, X, Check, Eye, EyeOff, Loader2 } from "lucide-react";
...
const CATEGORIES = ["general", "company", "projects", "industry", "events"];
...
export default function NewsClient({ initialNews }: { initialNews: NewsItem[] }) {
  ...
  const handleTitleChange = (title: string) => {
    const slug = title
      ...
      .replace(/[^\w\-а-яё]/gi, "")
      .replace(/--+/g, "-");
    setForm((prev) => ({ ...prev, title, ...(!editItem ? { slug } : {}) }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let res;
      if (editItem) {
        res = await fetch(`/api/admin/news/${editItem.id}`, {
          method: "PUT",
      ...
      if (editItem) {
        setNews((prev) => prev.map((n) => (n.id === editItem.id ? data.data : n)));
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 mb-1">Новости</h1>
          <p className="text-gray-500 text-sm">Управление статьями и новостями · {news.length} статей</p>
        </div>
        <button
          onClick={openCreate}
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-gray-400 text-xs mt-0.5 line-clamp-1">{item.excerpt || item.slug}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-16 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-gray-400">Новости пока не добавлены</p>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            ...
            <h3 className="text-lg font-bold text-gray-900 text-center mb-2">Удалить новость?</h3>
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl my-8">
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <div>
                <h2 className="text-xl font-bold text-gray-900">
                  {editItem ? "Редактировать новость" : "Новая статья"}
                </h2>
                <p className="text-sm text-gray-500 mt-0.5">
                  {editItem ? "Измените данные новости" : "Создайте новую статью"}
                </p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div className="sm:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">Контент *</label>
                  <textarea
                    required
                    value={form.content}
                ...
                <div className="sm:col-span-2">
                  <label className="flex items-center gap-2.5 cursor-pointer">
                    ...
                    <span className="text-sm text-gray-700 font-medium">Опубликована</span>
                  </label>
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={loading} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 font-medium text-sm disabled:opacity-70">
                  ...
                  {editItem ? "Сохранить" : "Создать новость"}
                </button>
              </div>
            </form>
          </div>
